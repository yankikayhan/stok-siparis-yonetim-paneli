...
- **Toplam urun:** Tum urunlerin sayisi.
- **Dusuk stok:** `stock` degeri `reorderLevel` degerine esit veya ondan az olan urunler.
...
| --- | --- | --- |
| `GET` | `/products` | Tum urunleri getirir. |
...
| `PATCH` | `/orders/:id` | Siparisin sadece durumunu gunceller. |
...
Mevcut ornek veri `server/db.json` icindedir: 3 kategori, 4 urun, 2 musteri, 2 siparis ve 1 profil kaydi bulunur.
...
API uzerinden yaptiginiz olusturma, duzenleme ve silme islemleri `server/db.json` dosyasina yazilir. Bu nedenle sunucuyu kapatip acmak degisiklikleri geri almaz. Baslangic verisine donmek icin bu dosyayi surum kontroldeki ilk haline geri dondurmeniz veya elinizle duzenlemeniz gerekir.
...
src/
  ...
  features/
    ...
    products/                  Urun API'si, sayfasi ve urun formu
    customers/                 Musteri API'si ve detay sayfasi
    orders/                    Siparis API'si, formu, sayfasi ve testleri
    settings/                  Profil API'si ve ayarlar sayfasi
  shared/
    api/                       Ortak HTTP istemcisi ve ApiError
    stores/ui-store.ts         Kalici global UI tercihleri
server/
  server.cjs                   json-server kurulumu ve ozel siparis endpoint'i
  db.json                      Kalici mock veri kaynagi
```
...
Mevcut Vitest testleri siparis formunun kritik stok kurallarini kapsar:
