  ...
  return (
    <div className="grid gap-3 border border-slate-200 bg-white p-4 lg:grid-cols-[minmax(0,1fr)_12rem_12rem] dark:border-slate-700 dark:bg-slate-900">
      <label className="relative block">
        ...
        <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" size={18} aria-hidden="true" />
        <input
          value={search}
          onChange={(event) => onSearchChange(event.target.value)}
          placeholder="Urun adi veya SKU ara"
          className="h-10 w-full border border-slate-300 bg-white pl-10 pr-3 text-sm outline-none placeholder:text-slate-400 focus:border-brand-700 focus:ring-2 focus:ring-brand-100 dark:border-slate-600 dark:bg-slate-900 dark:placeholder:text-slate-500 dark:focus:ring-brand-900"
        />
      </label>
      <label>
